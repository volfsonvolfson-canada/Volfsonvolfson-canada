<?php
// Server-Side Rendering for Vrienden page
require_once 'common.php';

// Redirect from .html to .php if accessed via old URL
if (basename($_SERVER['PHP_SELF']) === 'room-first-twin.html' || 
    (isset($_SERVER['REQUEST_URI']) && strpos($_SERVER['REQUEST_URI'], 'room-first-twin.html') !== false)) {
    header('Location: room-first-twin.php', true, 301);
    exit;
}
...
$title = safeOutput(
    btb_field_or_default($content, 'room_ground_twin_title', 'content_settings.room_ground_twin_title', 'Vrienden'),
    ''
);
$roomGalleryTitlePlain = trim((string) ($content['room_ground_twin_title'] ?? ''));
if ($roomGalleryTitlePlain === '') {
    $roomGalleryTitlePlain = btb_default_text('content_settings.room_ground_twin_title', 'Vrienden');
}
$subtitle = safeOutputWithBreaks(
    btb_field_or_default($content, 'room_ground_twin_subtitle', 'content_settings.room_ground_twin_subtitle', 'Perfect for friends or colleagues. Near the kitchen and massage hall.'),
    ''
);
$description = safeOutputWithBreaks(
    btb_field_or_default($content, 'room_ground_twin_description', 'content_settings.room_ground_twin_description', 'This first-floor room features two single beds, making it ideal for friends or colleagues traveling together. A shared bathroom with a large bathtub is located nearby.'),
    ''
);
$price = btb_room_price_line_html_stored_only($content, 'ground_twin');
$capacity = isset($content['room_ground_twin_capacity']) && !empty(trim($content['room_ground_twin_capacity'])) 
    ? safeHtmlOutput($content['room_ground_twin_capacity']) 
    : '<strong>Capacity:</strong> up to 2 guests';
$note = safeOutputWithBreaks(
    btb_field_or_default($content, 'room_ground_twin_note', 'content_settings.room_ground_twin_note', '*All tenants may use the sauna and home theatre free of charge, as long as it does not disturb other guests.'),
    ''
);

// Banner image
$bannerImageUrl = isset($content['room_ground_twin_banner_image_url']) && !empty(trim($content['room_ground_twin_banner_image_url'])) 
    ? safeOutput($content['room_ground_twin_banner_image_url'], '') 
    : '';

// Gallery (supports string URLs, object rows, double-encoded JSON)
$galleryJson = $content['room_ground_twin_gallery'] ?? '[]';

$tRoomGalH = trim((string) ($content['room_ground_twin_gallery_section_title'] ?? ''));
$roomGallerySectionHeading = htmlspecialchars(
    $tRoomGalH !== '' ? $tRoomGalH : btb_default_text('content_settings.room_ground_twin_gallery_section_title', 'Room photos'),
    ENT_QUOTES,
    'UTF-8'
);
$tCommonGalH = trim((string) ($content['room_ground_twin_common_gallery_section_title'] ?? ''));
$commonGallerySectionHeading = htmlspecialchars(
    $tCommonGalH !== '' ? $tCommonGalH : btb_default_text('content_settings.room_ground_twin_common_gallery_section_title', 'Common areas photos'),
    ENT_QUOTES,
    'UTF-8'
);
$commonGalleryJson = $content['room_ground_twin_common_gallery'] ?? '[]';
$commonGalleryAltPlain = trim((string) ($content['room_ground_twin_common_gallery_section_title'] ?? ''));
if ($commonGalleryAltPlain === '') {
    $commonGalleryAltPlain = btb_default_text('content_settings.room_ground_twin_common_gallery_section_title', 'Common areas');
}
...
<?php require_once __DIR__ . '/site-head-consent.php'; ?>
  ...
  <title>Vrienden | Back to Base</title>
  <?php
  $__seo_title = 'Vrienden | Back to Base';
  $__seo_desc = 'Vrienden twin room at Back to Base — ideal for two guests near kitchen and massage hall in Nelson, BC.';
  ?>
  <meta name="description" content="<?php echo htmlspecialchars($__seo_desc, ENT_QUOTES, 'UTF-8'); ?>">
  <?php
  btb_seo_emit_link_and_meta('/room-first-twin.php', $__seo_title, $__seo_desc, [
      'og_image' => '/assets/forest-deer.jpg',
  ]);
  ?>
  ...
  <section class="hero-contained" aria-label="Room banner">
    <div class="container">
      <?php if (!empty($bannerImageUrl)): ?>
        <img id="room-ground-twin-banner" 
      ...
      <?php else: ?>
        ...
        <img id="room-ground-twin-banner" 
             class="hero-contained-img basement-banner-cropped" 
             src="assets/forest-deer.jpg" 
             alt="Deer in forest"
             loading="eager"
             onerror="this.onerror=null; this.src='assets/forest-deer.jpeg'; this.onerror=function(){this.src='assets/forest-deer.JPG'; this.onerror=function(){this.src='assets/forest-deer.JPEG';};};" />
      <?php endif; ?>
    </div>
  </section>
  ...
  <main class="section">
    <div class="container room-hero">
      <section>
        ...
        <div class="gallery" id="room-ground-twin-gallery">
        ...
        <div class="gallery" id="room-ground-twin-common-gallery">
      ...
      <aside class="card room-booking-card" id="booking">
        ...
        <?php $btbRoomPricing = btb_room_booking_public_pricing_context($conn, $content, 'ground_twin', 'Vrienden'); ?>
        <form class="booking-form" data-room="Vrienden" data-custom-handler novalidate style="margin-top: 24px;"<?php echo $btbRoomPricing['data_attrs']; ?>>
          <div class="form-row">
            <div>
  ...
  <section id="wellness-section" class="section alt" style="display:none;">
    <div class="container">
      ...
      <section class="card card-massage">
        ...
        <div class="card-body">
          <h2 id="wellness-massage-title">Wellness</h2>
...
<?php require __DIR__ . '/site_footer.php'; ?>
  ...
  <script>
    ...
    (function() {
      // Run immediately before script.js loads
      const banner = document.getElementById('room-ground-twin-banner');
      if (banner) {
        // Mark banner as SSR-loaded to prevent script.js from processing it
        banner.setAttribute('data-ssr-loaded', 'true');
        banner.setAttribute('data-no-resolve', 'true'); // Prevent script.js resolveImages from processing
        banner.removeAttribute('data-src-base'); // Remove if present
        
        // Store original src
        const originalSrc = banner.src;
        
        // Override src setter to prevent unwanted updates
        let srcValue = originalSrc;
        Object.defineProperty(banner, 'src', {
          get: function() { return srcValue; },
          set: function(value) {
            // Only allow updates if it's a legitimate new image from admin (not from script.js resolveImages)
            if (value && 
                value !== originalSrc && 
                !value.includes('data-src-base') && 
                !value.includes('forest-deer') &&
                value.startsWith('assets/') &&
                (value.includes('banner') || value.includes('ground-twin'))) {
              srcValue = value;
              banner.setAttribute('src', value);
            }
        ...
        // Also prevent resolveImages from processing this image
        if (typeof window !== 'undefined') {
          window.__preventResolveImages = window.__preventResolveImages || [];
          window.__preventResolveImages.push('room-ground-twin-banner');
        }
      }
    })();
  </script>
