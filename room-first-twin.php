...
$title = safeOutput($content['room_ground_twin_title'] ?? '', 'Vrienden');
$roomGalleryTitlePlain = trim((string) ($content['room_ground_twin_title'] ?? ''));
if ($roomGalleryTitlePlain === '') {
    $roomGalleryTitlePlain = 'Vrienden';
}
$subtitle = safeOutputWithBreaks($content['room_ground_twin_subtitle'] ?? '', 'Perfect for friends or colleagues. Near the kitchen and massage hall.');
$description = safeOutputWithBreaks($content['room_ground_twin_description'] ?? '', 'This first-floor room features two single beds, making it ideal for friends or colleagues traveling together. A shared bathroom with a large bathtub is located nearby.');
$price = btb_room_price_line_html($content, 'ground_twin', btb_room_price_default_line_html('ground_twin'));
...
$note = safeOutputWithBreaks($content['room_ground_twin_note'] ?? '', '*All tenants may use the sauna and home theatre free of charge, as long as it does not disturb other guests.');
